...
class VelocityPass extends postprocessing.Pass {
  constructor(scene, camera) {
    ...
    this._scene = scene;
    this._camera = camera;
    this.renderTarget = new three.WebGLRenderTarget(((_window = window) == null ? void 0 : _window.innerWidth) || 1000, ((_window2 = window) == null ? void 0 : _window2.innerHeight) || 1000, {
      type: three.HalfFloatType
    });
  }
...
const zeroVec2 = new three.Vector2(); // the following variables can be accessed by the custom compose shader:
...
class TemporalResolvePass extends postprocessing.Pass {
  ...
  setupFramebuffers(width, height) {
    ...
    this.lastVelocityTexture = new three.FramebufferTexture(width * this.velocityResolutionScale, height * this.velocityResolutionScale, three.RGBAFormat);
...
function useBoxProjectedEnvMap(shader, envMapPosition, envMapSize) {
  ...
  shader.fragmentShader = boxProjectDefinitions + "\n" + shader.fragmentShader.replace("#include <envmap_physical_pars_fragment>", three.ShaderChunk.envmap_physical_pars_fragment).replace(line1, `vec3 worldNormal = inverseTransformDirection( normal, viewMatrix );
...
class SSREffect extends postprocessing.Effect {
  ...
  constructor(scene, camera, options = defaultSSROptions) {
    super("SSREffect", finalFragmentShader, {
      ...
      uniforms: new Map([["reflectionsTexture", new three.Uniform(null)], ["blur", new three.Uniform(0)], ["blurSharpness", new three.Uniform(0)], ["blurKernel", new three.Uniform(0)]]),
    ...
    this.cubeCamera = new three.CubeCamera(0.001, 1000, defaultCubeRenderTarget);
    ...
    if (!this.usingBoxProjectedEnvMap && this._scene.environment) {
      ...
      this._scene.traverse(c => {
        if (!envMap && c.material && !c.material.envMap) {
          ...
          if ("envMap" in properties && properties.envMap instanceof three.Texture) envMap = properties.envMap;
        }
      });
  ...
  static patchDirectEnvIntensity(envMapIntensity = 0) {
    if (envMapIntensity === 0) {
      three.ShaderChunk.envmap_physical_pars_fragment = three.ShaderChunk.envmap_physical_pars_fragment.replace("vec3 getIBLRadiance( const in vec3 viewDir, const in vec3 normal, const in float roughness ) {", "vec3 getIBLRadiance( const in vec3 viewDir, const in vec3 normal, const in float roughness ) { return vec3(0.0);");
    } else {
      three.ShaderChunk.envmap_physical_pars_fragment = three.ShaderChunk.envmap_physical_pars_fragment.replace("vec4 envMapColor = textureCubeUV( envMap, reflectVec, roughness );", "vec4 envMapColor = textureCubeUV( envMap, reflectVec, roughness ) * " + envMapIntensity.toFixed(5) + ";");
    }
  }
