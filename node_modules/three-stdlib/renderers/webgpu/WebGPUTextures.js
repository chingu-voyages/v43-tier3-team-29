...
class WebGPUTextures {
  ...
  initRenderTarget(renderTarget) {
    ...
    if (renderTargetProperties.initialized === undefined) {
      ...
      renderTargetProperties.colorTextureFormat = colorTextureFormat; // When the ".texture" or ".depthTexture" property of a render target is used as a map,
  ...
  _createTexture(texture) {
    ...
    const textureGPU = device.createTexture(textureGPUDescriptor); // transfer texture data
  ...
  _copyImageBitmapToTexture(image, textureGPU, origin = {
    x: 0,
    y: 0,
    z: 0
  }) {
